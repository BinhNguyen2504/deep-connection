...
DC.updatePrevBtn = function updatePrevBtn() {
	if (DC.refs.btnPrev) DC.refs.btnPrev.disabled = DC.state.usedQuestions.length === 0;
};
	...
	try {
		...
		if (type === "flip") {
			...
			[
				...
				o.frequency.exponentialRampToValueAtTime(freq * 0.85, now + delay + 0.12);
			...
			[0, 0.07, 0.14, 0.21].forEach((delay, i) => {
				...
				o.frequency.exponentialRampToValueAtTime(baseFreq * 0.75, now + delay + 0.06);
				g.gain.setValueAtTime(0, now + delay);
