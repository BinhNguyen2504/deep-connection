...
// ────────────── CATEGORY FILES ──────────────
DC.CATEGORY_FILES = ["first-sparks", "understanding", "passion", "bonding", "challenges", "future"];

// ────────────── FEEDBACK SESSION DATA ──────────────
DC._feedbackSession = {};
DC._votedSession = {};

DC.recordFeedback = function recordFeedback(categoryId, questionId, type, previousType) {
...
DC.getUserVote = function getUserVote(categoryId, questionId) {
	return (DC._votedSession[categoryId] && DC._votedSession[categoryId][String(questionId)]) || null;
};

DC.sendFeedbackWebhook = function sendFeedbackWebhook(categoryId, questionId, type) {
...
DC.getFeedbackCounts = function getFeedbackCounts(categoryId, questionId) {
	return (
		(DC._feedbackSession[categoryId] && DC._feedbackSession[categoryId][String(questionId)]) || {
			likes: 0,
			dislikes: 0
		}
	);
...
DC.bindFeedbackEvents = function bindFeedbackEvents(card, questionId, categoryId) {
	...
	function handleVote(btn, type, otherBtn, otherType) {
		btn.addEventListener("click", (e) => {
			...
			const counts = DC.recordFeedback(categoryId, questionId, type, previousType);
